...
#define CMD_GEM_BUILD	0x2000U /*GEM*/
#define CMD_GEM_REMOVE	0x4000U /*GEM*/
...
"  --builg-gem  -G chain \n"
"				Build gem structure of chain and start using it\n"
"  --remove-gem -Y chain \n"
"				Remove gem structure of chain and stop searching it\n"
...
int
build_gem(const ipt_chainlabel chain, int verbose,
	     iptc_handle_t *handle)
{
	if (verbose)
		fprintf(stdout, "Building chain `%s' gem structure\n", chain);
	return iptc_build_gem(chain, handle);
}
...
int do_command(int argc, char *argv[], char **table, iptc_handle_t *handle)
{
	...
	while ((c = getopt_long(argc, argv,
	   "-A:D:R:I:L::M:F::Z::N:X::G:Y::E:P:Vh::o:p:s:d:j:i:fbvnt:m:xc:",
					   opts, NULL)) != -1) {
		switch (c) {
			/*
			 * Command selection
		...
		case 'G':
			add_command(&command, CMD_GEM_BUILD, CMD_NONE,
				    invert);
			chain = optarg;
			fprintf(stdout, "GEM of `%s'\n", chain);
			break;

		case 'Y':
			add_command(&command, CMD_GEM_REMOVE, CMD_NONE,
				    invert);
			chain = optarg;
			break;
	...
	case CMD_ZERO:
		...
		break;
	case CMD_GEM_BUILD:
		ret = build_gem(chain, options&OPT_VERBOSE, handle);
		break;
