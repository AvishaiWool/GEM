...
#define STRUCT_IPINFO		struct ipt_ip
#define STRUCT_TCPINFO		struct ipt_tcp
#define STRUCT_INADDR		struct in_addr
...
#define TC_GET_IP_ADDR	get_ip_addr
...
STRUCT_IPINFO *get_ip_info(STRUCT_ENTRY *e)
{
	return &(e->ip);
}
...
void get_ip_addr(STRUCT_INADDR addr1, STRUCT_INADDR addr2, unsigned int *l_item, unsigned int *r_item)
{
	*l_item = htonl(addr1.s_addr&addr2.s_addr) ;
	*r_item = (unsigned int)(*l_item + htonl(~addr2.s_addr));
	//printf("Entry %u %u\n", *l_item, *r_item);
}
